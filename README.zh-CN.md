...
一个给 [Nous Research `hermes-agent`](https://github.com/nousresearch/hermes-agent) 用的 **AI agent skill**(也能作为独立 shell 脚本运行),在 macOS 上驱动 **LINE** 与 **WhatsApp** 桌面版 —— 发消息、发文件、拨语音/视频电话、读取与列出会话、回复指定消息。
...
- [ ] **微信 (WeChat)** 桌面自动化 —— 与 LINE/WhatsApp 相同的 UI/AX 路线(个人消息没有官方 API)。可能像 LINE 需要截图+视觉;注意微信反自动化较严,封号风险更高。
...
本项目通过桌面 App 的界面自动化个人 LINE/WhatsApp 账号,**仅供个人自用**,按现状提供、不附任何担保。

- 自动化个人账号可能**违反 WhatsApp / LINE 服务条款**,常见后果是账号被停用。风险自负。
- **请勿**用于 spam、大量或未经请求的消息、冒充他人,或未经同意读取/收集他人消息 —— 这些除了违反服务条款,还可能触犯反垃圾信息法与隐私法。
- 商业或大规模用途,请咨询律师并改用官方 API(WhatsApp Business Cloud API、LINE Messaging API)。
